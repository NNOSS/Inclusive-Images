...
def return_rep(WHICH_MODEL,x,classes,v, i,i2):
    if WHICH_MODEL == 0:
        batch = BatchNormLayer(x,act=tf.nn.relu, is_train=True,name='gen_bn_%i_%i'%(i,i2))
        batch = feature_concat(batch.outputs, classes)
    elif WHICH_MODEL ==1:
        batch = batch_norm_sota(x.outputs,classes, v, i,i2)
    elif WHICH_MODEL == 2:
        batch = batch_norm_map(x.outputs,classes, v, i,i2)
    return batch

def batch_norm_map(x,classes,v, i,i2):
    ...
    with tf.variable_scope('batch_norm_%i_%i'%(i,i2)) as scope:
        ...
        class_matrix = tf.get_variable('gen_mapping_w_%i_%i'%(i,i2), [NUM_CLASSES, G_PROJECTIONS[i]])
        bias_matrix = tf.get_variable('gen_mapping_b_%i_%i'%(i,i2), [NUM_CLASSES, G_PROJECTIONS[i]])
        ...
        batch_gamma = tf.layers.dense(z2, abs(v), name='gen_gamma_projection_%i_%i'%(i,i2))
        batch_beta = tf.layers.dense(z2, abs(v), name='gen_beta_projection_%i_%i'%(i,i2))
...
def batch_norm_disc(x,classes,v, i,i2):
    ...
    with tf.variable_scope('batch_norm_%i_%i'%(i,i2)) as scope:
        ...
        mean, variance = tf.nn.moments(x, axes=[0,1,2])
        x_normed = (x - mean) / tf.sqrt(variance + epsilon)
        class_matrix = tf.get_variable('d_mapping_w_%i_%i'%(i,i2), [NUM_CLASSES, abs(v)])
        bias_matrix = tf.get_variable('d_mapping_b_%i_%i'%(i,i2), [NUM_CLASSES, abs(v)])
        class_selection = tf.expand_dims(class_max, -1)
...
def batch_norm_sota(x,classes,v, i,i2):
    '''manual batch norm because tensorflow can't set beta or gamma'''
    with tf.variable_scope('batch_norm_%i_%i'%(i,i2)) as scope:
        epsilon = 1e-8
        mean, variance = tf.nn.moments(x, axes=[0,1,2])
